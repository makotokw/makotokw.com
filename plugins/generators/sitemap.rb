...
# Modified for makotokw.com by Makoto Kawasaki
#
...
module Jekyll
  ...
  EXCLUDED_FILES = %w(atom.xml 404.html 50x.html error.html robots.txt)

  # Any files that include posts, so that when a new post is added, the last
  # modified date of these pages should take that into account
  PAGES_INCLUDE_POSTS = %w(index.html)
  ...
  CHANGE_FREQUENCY_VAR_NAME = 'change_frequency'
  ...
  class SitemapGenerator < Generator
    ...
    def fill_url(site, page_or_post)
      ...
      if page_or_post.data[CHANGE_FREQUENCY_VAR_NAME]
        change_frequency =
            page_or_post.data[CHANGE_FREQUENCY_VAR_NAME].downcase
      ...
      if page_or_post.data[PRIORITY_VAR_NAME]
        priority_value = page_or_post.data[PRIORITY_VAR_NAME]
        if valid_priority?(priority_value)
          priority = REXML::Element.new 'priority'
          priority.text = page_or_post.data[PRIORITY_VAR_NAME]
